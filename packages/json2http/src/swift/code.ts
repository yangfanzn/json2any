...
export class Http extends Base.Http<Complex, Simple> {
  toLaunch(plan: Base.SchemaPlan) {
    ...
    return {
      code: `
    func ${this.launch}(_ setPlan: (${this.declPlan}) -> Void) async throws -> ${this.declPlan} {
...
class AlamofireAgent: Agent {
  fileprivate static let session = Session(configuration: {
  ...
  func fetch${addX('P: Plan')}(plan: P) async throws -> Reply {
    var plan = plan
    let session = self.session ?? Self.session
    ...
    plan.reply.code = response.response?.statusCode
    plan.reply.message = _code2message["\\(plan.reply.code ?? 0)"] ?? "unknown http code \\(plan.reply.code)"
    
    switch response.result {
      case .success(let data): plan.reply.data = (try? _parse(data)) ?? data
  ...
  func body${addX('P: Plan')}(plan: P) async throws -> Any? {
    ...
    switch type {
      ...
      case "json":
        return _stringify(
          (data as? [Any])?.map { ($0 as? Json2class).map { _nullFilter($0.toJson()) } ?? $0 } 
          ?? (data as? Json2class).map { _nullFilter($0.toJson()) } ?? data
        ).data(using: .utf8)
      case "form":
        guard let formData = data as? BodyFormProtocol else {
          return nil
        }
        let map = MultipartFormData()
          ...
          for e in values {
            if let file = e as? BodyFormFile {
              ...
              if let fieldData = "\\(e)".data(using: .utf8) {
                map.append(fieldData, withName: key)
              }
            }
          }
        }
        formData._fields.toJson().forEach(cb)
        formData._files.toJson().forEach(cb)
        return map               
      default: return nil
    }
    return nil
...
class NilJson2class: Json2class {
...
class BodyFormFile {
  ...
  static func fromFile(_ file: String) -> BodyFormFile { return BodyFormFile(content: nil, file: file) }
  static func fromString(_ value: String) -> BodyFormFile { return BodyFormFile(content: value.data(using: .utf8), file: nil) }
  static func fromBytes(_ value: Data) -> BodyFormFile { return BodyFormFile(content: value, file: nil) }
}

fileprivate protocol BodyFormProtocol { var _fields: Json2class { get }; var _files: Json2class { get } }
class BodyForm${addX('T: Json2class, K: Json2class')}: BodyFormProtocol {
  ...
  fileprivate var _fields: Json2class { return fields }
  fileprivate var _files: Json2class { return files }
}

fileprivate let _body2type: [String: String] = [${Base.bodyTypes
...
extension Plan {
  ...
  mutating func request() async throws -> Void {
    ...
    self.res.fromAny(self.reply.data)
