...
private fun _parse(data: String): Any? {
  fun convert(item: Any?): Any? {
    return when (item) {
      is _JSONObject -> {
      ...
      is _JSONArray -> {
        ...
        for (i in 0 until item.length()) {
            list.add(convert(item.get(i)))
        }
        list
      }
      _JSONObject.NULL -> null
      else -> item
    }
  }
  val trimmed = data.trim()
  if (trimmed.isEmpty()) return data
  val tokener = _JSONTokener(trimmed)
  val value = tokener.nextValue()
  if (tokener.more()) {
    val remainingChar = tokener.nextClean()
    if (remainingChar != 0.toChar()) {
      throw Exception("Unexpected character at end of JSON input: $remainingChar")
    }
  }
...
abstract class Json2class {
  ...
  fun fromAny(data: Any?, setRule: ((Rule) -> Unit)? = null, rule: Rule? = null): Json2class {
    var _data = data
    try {
      _data = _parse(_data.toString())
    } catch (_: Exception) {}
    return fromJson(_data, setRule, rule)
  }

  abstract fun fromJson(data: Any?, setRule: ((Rule) -> Unit)? = null, rule: Rule? = null): Json2class

  open var preset = ""

  fun fromPreset(setRule: ((Rule) -> Unit)? = null, rule: Rule? = null): Json2class {
  ...
  private fun _isSameSimple(source: Any?, target: Any?): Boolean {
    return source?.javaClass == target?.javaClass || (source is Number && target is Number)
  }
        ...
        if (!isExist) {
          if (rule.moreIndex == MoreIndex.Null && array[level - 1]) {
            ...
            if (rule.moreIndex == MoreIndex.Drop) {
              ...
              t.add(if (array[level - 1]) null else _nList<T>(array, array.size - level))
            }
          }
        } else if (_data is List<Any?>) {
          t.add(_nArray<T>(
